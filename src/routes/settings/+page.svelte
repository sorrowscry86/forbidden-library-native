...
<div class="container mx-auto px-6 py-8 max-w-4xl">
  ...
  {:else}
    ...
    <div class="bg-gray-800 rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <p class="text-white">
            {$environment === 'tauri' ? 'Desktop Application' : 'Web Browser'}
          </p>
        </div>
    ...
    <div class="bg-gray-800 rounded-lg p-6 mt-8">
      ...
      <button
        on:click={async () => {
          // Fetch and display environment information
          environmentInfo = getEnvironmentInfo();
          alert(JSON.stringify(environmentInfo, null, 2)); // Display in a readable format
        }}
      ...
      {#if Object.keys(environmentInfo).length > 0}
        <pre class="text-white mt-4">{JSON.stringify(environmentInfo, null, 2)}</pre>
      {/if}
    </div>
