<script lang="ts">
  ...
  let environmentInfo: ReturnType<typeof getEnvironmentInfo> | null = null;
  ...
  async function performHealthCheck() {
    try {
      const [apiHealth, envHealth] = await Promise.all([healthCheck(), environmentHealthCheck()]);

      systemHealth =
        apiHealth.status === 'healthy' && envHealth.status === 'healthy'
          ? 'healthy'
  ...
  async function loadConversations() {
    try {
      ...
      conversations = await invokeWithIntelligentRetry<Conversation[]>('get_conversations', args, {
        maxRetries: 2,
        baseDelayMs: 1000,
...
<div class="bg-gray-900 border-b border-gray-700">
  ...
  {#if $environment === 'web'}
    <div class="bg-yellow-600 text-white px-4 py-2 text-center text-sm">
      ...
      <a href="https://tauri.app/" target="_blank" rel="noopener noreferrer" class="underline ml-2">
        Install Desktop App
      </a>
    </div>
  {/if}
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              {#each Object.entries(diagnosticsData.circuitBreakers) as [command, metrics]}
                <span
                  class="px-2 py-1 rounded text-xs {metrics.state === 'closed'
                    ? 'bg-green-600'
                    : metrics.state === 'open'
                      ? 'bg-red-600'
                      : 'bg-yellow-600'}"
          ...
          {#if $environment === 'web'}
            <div class="bg-gray-700 rounded-lg p-4 mb-4">
              <p class="text-sm text-gray-300 mb-2">
                🌐 You're running in web mode. For the full experience with local storage, file
                system access, and enhanced privacy controls, run the desktop application.
              </p>
              <button
          ...
          {#if systemHealth !== 'healthy'}
            <div class="bg-yellow-900 border border-yellow-600 rounded-lg p-3 text-sm">
              ...
              <p class="text-yellow-200">
                Some features may be limited. The system is monitoring and attempting to recover
                automatically.
              </p>
            </div>
          {/if}
