...
export function isTauriAvailable(): boolean {
  try {
    ...
    // Require at least 1 out of 4 checks to pass for Tauri detection
...
export function monitorEnvironmentChanges(
  callback: (newEnv: 'tauri' | 'web' | 'unknown') => void
): () => void {
...
export async function safeInvoke<T>(
  ...
  if (environment === 'tauri') {
    ...
    throw new AppError({
      ...
      details:
        'Unable to determine the current environment. This might indicate a configuration issue.',
      category: ErrorCategory.ENVIRONMENT,
      severity: ErrorSeverity.ERROR,
...
export async function environmentHealthCheck(): Promise<{
  ...
  if (environmentInfo.confidence < 0.8) {
    issues.push(
      `Low environment detection confidence: ${Math.round(environmentInfo.confidence * 100)}%`
    );
  }
