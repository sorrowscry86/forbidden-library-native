<script lang="ts">
  ...
  async function addMilestone() {
    ...
    try {
      ...
      if (currentProject.metadata) {
        currentProject.metadata.milestones = [...currentProject.metadata.milestones, milestone];
      }
  ...
  function toggleMilestoneStatus(milestoneId: string) {
    ...
    const milestoneIndex = currentProject.metadata.milestones.findIndex(
      (m) => m.id === milestoneId
    );
                ...
                <div class="bg-gray-800 p-4 rounded-lg flex items-start">
                  <div class="mr-3 mt-1">
                    <button
                      ...
                      class="w-5 h-5 rounded-full border {milestone.completed
                  ...
                  <div class="flex-1">
                    <h4
                      class="text-white font-medium {milestone.completed
                    ...
                    <div class="flex items-center mt-2 text-xs text-gray-500">
                      ...
                      {#if milestone.completed && milestone.completed_at}
                        <span class="ml-3"
                          >Completed: {new Date(milestone.completed_at).toLocaleDateString()}</span
                        >
                      {/if}
                    </div>
                  </div>
          ...
          <div class="bg-gray-700 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div class="bg-gray-800 p-4 rounded-lg">
                ...
                <p class="text-white font-semibold">
                  {currentProject.metadata.repository_stats.total_commits}
                </p>
              </div>
              ...
              <div class="bg-gray-800 p-4 rounded-lg">
                ...
                <p class="text-white font-semibold">
                  {currentProject.metadata.repository_stats.total_lines.toLocaleString()}
                </p>
              </div>
              ...
              <div class="bg-gray-800 p-4 rounded-lg">
                ...
                <p class="text-white font-semibold">
                  {new Date(
                    currentProject.metadata.repository_stats.last_commit_date
                  ).toLocaleDateString()}
                </p>
              </div>
            </div>
                  ...
                  <div>
                    <div class="flex justify-between text-xs mb-1">
                      ...
                      <span class="text-gray-400"
                        >{lines.toLocaleString()} lines ({Math.round(
                          (lines / currentProject.metadata.repository_stats.total_lines) * 100
                        )}%)</span
                      >
                    </div>
