<script lang="ts">
    ...
    try {
      ...
      if (result) {
        ...
        await safeInvoke('write_file_to_disk', {
          path: result,
          content:
            'This is a sample conversation export from Forbidden Library desktop app.\n\nThis demonstrates native file system access!',
        });
  ...
  async function openUrl() {
    try {
      await safeInvoke('open_external_url', {
        url: 'https://github.com/sorrowscry86/forbidden-library',
      });
    } catch (error) {
...
<div class="desktop-features">
  ...
  {#if !isDesktop}
    <div class="web-notice">
      ...
      <p>
        You're currently using the web version. Download and install the desktop application to
        access these native features:
      </p>
      <ul>
  ...
  {:else}
    ...
    <div class="feature-group">
      ...
      <div class="button-grid">
        <button on:click={openFileDialog} class="feature-button"> 📂 Open File Dialog </button>
        <button on:click={saveFileDialog} class="feature-button"> 💾 Save File Dialog </button>
        <button on:click={getAppDataDir} class="feature-button"> 📁 App Data Directory </button>
      </div>
    </div>
    ...
    <div class="feature-group">
      ...
      <div class="button-grid">
        <button on:click={showNotification} class="feature-button"> 🔔 Show Notification </button>
        <button on:click={copyToClipboard} class="feature-button"> 📋 Copy to Clipboard </button>
        <button on:click={readFromClipboard} class="feature-button"> 📄 Read Clipboard </button>
        <button on:click={openUrl} class="feature-button"> 🌐 Open External URL </button>
      </div>
    ...
    <div class="feature-group">
      ...
      <div class="button-grid">
        <button on:click={toggleAlwaysOnTop} class="feature-button {alwaysOnTop ? 'active' : ''}">
          📌 Always On Top {alwaysOnTop ? '(ON)' : '(OFF)'}
        </button>
        <button on:click={minimizeToTray} class="feature-button"> ⬇️ Minimize to Tray </button>
      </div>
