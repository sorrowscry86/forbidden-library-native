<script lang="ts">
  ...
  async function loadMessages() {
    ...
    try {
      loading = true;
      messages = await invokeWithTimeout<Message[]>(
        'get_messages',
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const userMsg = await invokeWithTimeout<Message>(
        'add_message',
        {
          conversation_id: conversation.id,
          role: 'user',
      ...
      const aiResponse = await invokeWithTimeout<AiResponse>(
        'send_ai_request',
        {
          message: userMessage,
      ...
      const aiMsg = await invokeWithTimeout<Message>(
        'add_message',
        {
          conversation_id: conversation.id,
          role: 'assistant',
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center space-x-2">
        ...
        <button
          ...
          on:click={() => archiveConversation(conversation.id)}
          ><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
