...
use sentry::{start_transaction, Transaction, TransactionContext};
use std::sync::Arc;
...
impl ScopedTransaction {
    ...
    pub fn new(name: &str, op: &str) -> Self {
        let transaction = start_transaction(TransactionContext::new(name, op));
    ...
    pub fn finish(&mut self) {
        if let Some(transaction) = self.transaction.take() {
            Arc::try_unwrap(transaction).map(|t| t.finish()).ok();
        }
    }
}
