...
impl PerformanceMonitor {
    ...
    pub fn finish_startup_tracking(start_time: Instant, config: Option<&PerformanceConfig>) {
        ...
        if duration.as_millis() > threshold as u128 {
            error!(
                "❌ Startup time exceeded {} ms: {}ms",
                threshold,
                duration.as_millis()
            );
            sentry::capture_message(
                &format!(
                    "Slow startup: {}ms - VoidCat RDC Performance Alert",
                    duration.as_millis()
                ),
                sentry::Level::Error,
            );
            ...
            info!(
                "✅ Startup time: {}ms - VoidCat RDC Performance Standard",
                duration.as_millis()
            );
        }
    ...
    {
        let transaction = start_transaction(TransactionContext::new(
            &format!("db.{}", operation),
            "db.operation",
        ));
    ...
    pub fn track_ai_request<E: std::fmt::Display>(
        ...
        match status {
            Ok(_) => {
                if let Some(tokens) = tokens_used {
                    info!(
                        "🤖 AI request completed: {} model, {} tokens",
                        model, tokens
                    );
                } else {
