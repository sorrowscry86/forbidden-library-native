...
fn benchmark_conversation_creation(c: &mut Criterion) {
    ...
    c.bench_function("conversation_creation", |b| {
        b.iter(|| {
            let _conversation = services
                .conversations
                .create_conversation(black_box("Benchmark Conversation".to_string()), None)
    ...
    c.bench_function("bulk_conversation_creation_100", |b| {
        b.iter(|| {
            for i in 0..100 {
                let _conversation = services
                    .conversations
                    .create_conversation(format!("Benchmark Conversation {}", i), None)
                    .expect("Failed to create conversation");
            }
        });
    });
...
fn benchmark_api_config_retrieval(c: &mut Criterion) {
    ...
    c.bench_function("api_config_retrieval", |b| {
        b.iter(|| {
            let _config = services
                .apis
                .get_api_config("benchmark_provider")
    ...
    c.bench_function("concurrent_conversation_creation", |b| {
        b.iter(|| {
            ...
            let handle = std::thread::spawn(move || {
                for i in 0..10 {
                    let _conversation = services_clone
                        .conversations
                        .create_conversation(format!("Concurrent Conversation {}", i), None)
                        .expect("Failed to create conversation");
                }
            });
    ...
    c.bench_function("memory_usage_large_dataset", |b| {
        b.iter(|| {
            ...
            for i in 0..1000 {
                let conversation = services
                    .conversations
                    .create_conversation(format!("Memory Test Conversation {}", i), None)
                    .expect("Failed to create conversation");
...
fn benchmark_startup_time(c: &mut Criterion) {
    c.bench_function("startup_time", |b| {
        b.iter(|| {
            ...
            let db_manager = DatabaseManager::new_in_memory().expect("Failed to create database");
            ...
            for i in 0..10 {
                services
                    .conversations
                    .create_conversation(format!("Startup Conversation {}", i), None)
                    .expect("Failed to create conversation");
            }
...
fn benchmark_ui_responsiveness(c: &mut Criterion) {
    ...
    let conversation = services
        .conversations
        .create_conversation("UI Test Conversation".to_string(), None)
        .expect("Failed to create conversation");
    let conversation_id = conversation.id.unwrap();
...
fn benchmark_search_performance(c: &mut Criterion) {
    ...
    for i in 0..100 {
        services
            .conversations
            .create_conversation(format!("Search Test Conversation {}", i), None)
            .expect("Failed to create conversation");
    }
    ...
    c.bench_function("search_conversations", |b| {
        b.iter(|| {
            ...
            let filtered: Vec<_> = _conversations
                .into_iter()
...
fn benchmark_export_performance(c: &mut Criterion) {
    ...
    let conversation = services
        .conversations
        .create_conversation("Export Test Conversation".to_string(), None)
        .expect("Failed to create conversation");
    let conversation_id = conversation.id.unwrap();
