...
async fn test_sql_injection_prevention_conversations() {
    ...
    for malicious_input in malicious_inputs {
        let result = env
            .services
            .conversations
            .create_conversation(malicious_input.to_string(), None);

        // Should succeed but not cause SQL injection
        assert!(
            result.is_ok(),
            "Failed to handle malicious input: {}",
            malicious_input
        );
        ...
        assert!(
            ...
            "Table should still contain data after malicious input"
        );
    }
}
...
async fn test_path_traversal_prevention() {
    ...
    for path_attempt in &path_traversal_attempts {
        let result = env
            .services
            .conversations
            .create_conversation(path_attempt.to_string(), None);

        // Should succeed but not allow path traversal
        assert!(
            result.is_ok(),
            "Failed to handle path traversal attempt: {}",
            path_attempt
        );
...
async fn test_command_injection_prevention() {
    ...
    for injection_attempt in command_injection_attempts {
        let result = env
            .services
            .conversations
            .create_conversation(injection_attempt.to_string(), None);

        // Should succeed but not execute commands
        assert!(
            result.is_ok(),
            "Failed to handle command injection attempt: {}",
            injection_attempt
        );
...
async fn test_xss_prevention() {
    ...
    for xss_attempt in xss_attempts {
        let result = env
            .services
            .conversations
            .create_conversation(xss_attempt.to_string(), None);

        // Should succeed but not execute scripts
        assert!(
            result.is_ok(),
            "Failed to handle XSS attempt: {}",
            xss_attempt
        );
...
async fn test_input_validation_conversation_titles() {
    ...
    let result = env
        .services
        .conversations
        .create_conversation(long_title.clone(), None);
    ...
    let result = env
        .services
        .conversations
        .create_conversation(null_title.to_string(), None);
    ...
    let result = env
        .services
        .conversations
        .create_conversation(unicode_title.to_string(), None);
...
async fn test_persona_id_validation() {
    ...
    let result = env
        .services
        .conversations
        .create_conversation("Test Conversation".to_string(), Some(-1));

    assert!(
        result.is_ok(),
        "Should handle negative persona ID gracefully"
    );
    ...
    let result = env
        .services
        .conversations
        .create_conversation("Test Conversation".to_string(), Some(i64::MAX));
    ...
    let result = env
        .services
        .conversations
        .create_conversation("Test Conversation".to_string(), Some(0));
...
async fn test_pagination_parameter_validation() {
    ...
    let result = env.services.conversations.get_conversations(Some(-1), None);

    assert!(result.is_ok(), "Should handle negative limit gracefully");

    // Test negative offsets
    let result = env.services.conversations.get_conversations(None, Some(-1));
    ...
    let result = env
        .services
        .conversations
        .get_conversations(Some(i32::MAX), None);
    ...
    let result = env
        .services
        .conversations
        .get_conversations(None, Some(i32::MAX));
...
async fn test_conversation_id_validation() {
    ...
    assert!(
        result.is_ok(),
        "Should handle negative conversation ID gracefully"
    );
    ...
    assert!(
        result.is_ok(),
        "Should handle large conversation ID gracefully"
    );
    ...
    assert!(
        result.is_ok(),
        "Should handle zero conversation ID gracefully"
    );
}
...
async fn test_memory_exhaustion_prevention() {
    ...
    assert_eq!(
        conversations.len(),
        1000,
        "Should retrieve all conversations"
    );
}
...
async fn test_concurrent_access_security() {
    ...
    for i in 0..100 {
        ...
        let handle = task::spawn(async move {
            // Create conversation
            let conv_result = services_clone
                .conversations
                .create_conversation(format!("Concurrent Test {}", i), None);

            assert!(
                conv_result.is_ok(),
                "Concurrent conversation creation failed"
            );
            ...
            assert!(
                get_result.is_ok(),
                "Concurrent conversation retrieval failed"
            );
            ...
            assert!(
                delete_result.is_ok(),
                "Concurrent conversation deletion failed"
            );
        });
...
async fn test_error_handling_security() {
    ...
    assert!(
        result.is_err(),
        "Should return error for non-existent conversation deletion"
    );
...
async fn test_service_isolation() {
    ...
    let conv2 = env
        .services
        .conversations
        .create_conversation("Isolation Test 2".to_string(), None)
        .unwrap();
    ...
    env.services
        .conversations
        .delete_conversation(conv1.id.unwrap())
        .unwrap();

    let conv2_retrieved = env
        .services
        .conversations
        .get_conversation(conv2.id.unwrap())
        .unwrap();

    assert!(
        conv2_retrieved.is_some(),
...
async fn test_data_integrity_malicious_input() {
    ...
    for malicious_input in malicious_inputs {
        ...
        let retrieved_conv = env
            .services
            .conversations
            .get_conversation(legitimate_id)
            .unwrap();

        assert!(
            retrieved_conv.is_some(),
            "Legitimate conversation should still exist"
        );
    ...
    let all_conversations = env
        ...
        .unwrap();
    assert!(
        all_conversations.len() >= 4,
        "Should have at least 4 conversations (1 legitimate + 3 malicious)"
    );
}
