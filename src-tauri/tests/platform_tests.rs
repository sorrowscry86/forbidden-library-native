...
fn test_cross_platform_home_dir() {
    ...
    assert!(home.is_some(), "Home directory should be accessible on all platforms");
}

#[test]
fn test_cross_platform_temp_dir() {
    let temp = platform::get_temp_dir();
    assert!(temp.exists(), "Temp directory should exist on all platforms");
}

#[test]
fn test_cross_platform_app_data_dir() {
    let app_data = platform::get_app_data_dir();
    assert!(app_data.is_some(), "App data directory should be accessible on all platforms");
...
fn test_env_var_access() {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        assert!(username.is_some(), "USERNAME should be available on Windows");
    }
    
    #[cfg(not(target_os = "windows"))]
    {
        let user = platform::get_env_var("USER");
        assert!(user.is_some(), "USER should be available on Unix-like systems");
    }
}
...
mod unix_tests {
    ...
    fn test_unix_app_data_dir() {
        ...
        assert!(app_data.is_some(), "Unix app data directory should be accessible");
